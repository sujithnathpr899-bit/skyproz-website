...
SQLite requires persistent storage and cannot run as a plain static deployment. The root `package.json` and `render.yaml` deploy the company website and this module together as one Render Node.js Web Service, so `/` serves the Skyproz site and `/contract-finder/` serves this platform.

For Render, connect the repository as a Blueprint with no Root Directory. The service uses `npm install` as the build command and `npm start` as the start command. Mount the persistent disk at `/var/data`, keep `DATABASE_PATH=/var/data/contract-finder.db`, and set `APP_ORIGIN=https://skyproz.in` after the custom domain is connected. Startup runs migrations automatically.
